...
class HierarchicalFusion(nn.Module):
    ...
    def forward(self, feat_temporal, feat_frequency, feat_timefreq, return_intermediate=False):
        ...
        Args:
            ...
            return_intermediate: 是否返回中间特征（用于可视化分析）
        
        Returns:
            fused_feat: 融合特征 (B, output_dim)
            intermediate_dict: 中间特征字典（如果return_intermediate=True）
        """
        # ==================== Level 1: 模态重要性学习 ====================
        ...
        multi_granularity_feat = torch.cat([
            feat_temporal,      # 原始时域特征
            feat_frequency,     # 原始频域特征
            feat_timefreq,      # 原始时频特征
            weighted_feat,      # Level 1加权特征
            interact_feat       # Level 2交互特征
        ], dim=1)  # (B, 1280)
...
class HierarchicalFusionV2(nn.Module):
    ...
    def forward(self, feat_temporal, feat_frequency, feat_timefreq):
        ...
        for cross_modal_layer in self.cross_modal_layers:
            current_feat, _ = cross_modal_layer(
...
if __name__ == '__main__':
    print("Testing Hierarchical Fusion...")
    ...
    print(f"✓ Fused feature shape: {fused_feat.shape}")  # (8, 128)
    
    # 测试返回中间特征
    print("\n2. Testing with intermediate features...")
    ...
    print(f"\n✓ Sample modal weights: {intermediate['modal_weights'][0]}")
    
    # 测试V2版本
    print("\n3. Testing HierarchicalFusionV2...")
    ...
    print(f"✓ Fused feature V2 shape: {fused_feat_v2.shape}")
    
    # 参数统计
    print("\n4. Model statistics...")
    total_params = sum(p.numel() for p in fusion.parameters())
    print(f"✓ HierarchicalFusion parameters: {total_params:,}")
    
    total_params_v2 = sum(p.numel() for p in fusion_v2.parameters())
    print(f"✓ HierarchicalFusionV2 parameters: {total_params_v2:,}")
